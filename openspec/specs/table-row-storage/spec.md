# table-row-storage Specification

## Purpose

Where a settings table keeps its rows, and how much of the set a mutation is allowed to read and write. The option array a table edited by hand can live in cannot survive two writers, so the rows sit behind an adapter: one that keeps the option shape, one that gives each row its own database row, and a contract narrow enough that a handler can only touch the rows named in the request.
